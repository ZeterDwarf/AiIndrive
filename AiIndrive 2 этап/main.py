...
class ScoringEngine:
    def __init__(self):
        ...
        # Базовые колонки (ожидаемые). Будут корректироваться при глубоком анализе (Gate 2)
        self.cols = [
    ...
    def load_data(self):
        # Если в папке лежит реальный датасет от организаторов - читаем его
        if os.path.exists("dataset.csv"):
            print("Загружаем реальный датасет заказчика (dataset.csv)...")
            try:
                df = pd.read_csv("dataset.csv")
                # Для первого этапа - если названия колонок другие, заполняем нулями чтобы не падал
                for c in self.cols:
                    if c not in df.columns:
                        df[c] = 0.0
                if 'target' not in df.columns:
                    # Искуственная разметка для MVP, на 2 этапе сделаем нормальный label engineering
                    df['target'] = np.random.choice([0, 1], size=len(df))
                return df
            except Exception as e:
                print(f"Ошибка чтения dataset.csv ({e}). Переходим на mock-данные.")
                return self.get_mock_data()
        else:
            print("dataset.csv не найден. Генерируем тестовую выборку для MVP...")
            return self.get_mock_data()

    def get_mock_data(self, n=500):
        np.random.seed(42)
        df = pd.DataFrame({
